...
export async function updateOrganizationProfile(data: OrganizationData, id: number) {
  await axios.post('/api/job-portal/update-organization-profile', { data, id });
}
...
export async function inviteRecruiterToOrg(email: string, orgId: number) {
  const resp = await axios.post('/api/job-portal/create-invite-to-org', { email, orgId });
...
export async function registerRecruiter(
  ...
  const resp = await axios.post('/api/job-portal/register-recruiter', {
    name,
    email,
    position,
    companyName,
...
export async function deleteJobCategory(id: number, instanceId?: string) {
  await axios.post('/api/job-portal/delete-job-category', { id, instanceId });
}
...
export async function deleteJobPost(id: number) {
  await axios.post('/api/job-portal/delete-job-post', { id });
}
