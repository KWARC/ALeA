...
function autoParseJson(obj: unknown): unknown {
  if (obj === null || obj === undefined) return obj;
  ...
  if (typeof obj === 'string' || Buffer.isBuffer(obj)) {
    ...
    if ((str.startsWith('{') && str.endsWith('}')) || (str.startsWith('[') && str.endsWith(']'))) {
      try {
        return autoParseJson(JSON.parse(str));
      } catch {
        return obj;
      }
    }
...
const globalTransform = async ({ args, query }: any) => {
  ...
  return serializeBigInt(autoParseJson(result));
};
...
type ExtendedPrismaClient = ReturnType<typeof createCommentsDbClient>;

const globalForPrisma = globalThis as unknown as {
  commentsDb?: ExtendedPrismaClient;
};

export const commentsDb = globalForPrisma.commentsDb ?? createCommentsDbClient();
