...
import { SearchDialog } from '../course-notes';
...
export function setSlideNumAndSectionId(router: NextRouter, slideNum: number, sectionId?: string) {
  ...
  const instanceId = query.instanceIdOrLatest as string;
...
const CourseViewPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (rawInstitutionId !== institutionId) {
      ...
      const normalizedPath = `/${institutionId}/${courseId}/${instanceIdOrLatest}/course-view${queryString ? `?${queryString}` : ''}`;
