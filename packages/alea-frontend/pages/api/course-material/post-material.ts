...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (type === 'FILE') {
    ...
    try {
      ...
      const dbResult = await executeAndEndSet500OnError(
        `INSERT INTO CourseMaterials 
         (id, materialName, materialType, storageFileName, mimeType, sizeBytes,
          universityId, courseId, semesterId, uploadedBy, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          materialId,
          materialName,
      ...
      return res.status(200).json({ id: materialId });
    } catch (error) {
    ...
    const dbResult = await executeAndEndSet500OnError(
      `INSERT INTO CourseMaterials 
       (id, materialName, materialType, universityId, courseId, semesterId, uploadedBy, url, createdAt, updatedAt)
       VALUES (?, ?, 'LINK', ?, ?, ?, ?, ?, ?, ?)`,
      [
        materialId,
        materialName,
