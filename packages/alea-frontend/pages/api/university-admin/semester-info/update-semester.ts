...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const result = (await executeQuery(
    ...
    `,
    [semesterStart, semesterEnd, lectureStartDate, lectureEndDate, userId, universityId, instanceId]
  ...
  res
    .status(200)
    .send((result?.affectedRows ?? 0) > 0 ? 'Semester updated successfully' : 'No rows updated');
}
