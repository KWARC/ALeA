...
async function fetchPeerContributionCounts(
  ...
  const cacheKey = `${courseId}::${courseInstanceId}::${normalizedUserIds.join(',')}`;
  ...
  const loadPromise = (async (): Promise<Map<string, PeerContributionCounts>> => {
    try {
      const inList = normalizedUserIds.map(() => '?').join(', ');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const sqlParams: (string | number)[] = [courseId, courseInstanceId, graderUserId, graderUserId];
  ...
  const responses = await executeAndEndSet500OnError<
