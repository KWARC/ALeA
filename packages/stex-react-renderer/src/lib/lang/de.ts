import { ProblemFilter } from '../ProblemFilter';

export const de = {
  // content dashboard
  expandCollapseAll: 'Alles anzeigen/Minimieren',
  search: 'Suchen Inhaltsverzeichnis',

  // expandable context menu
  copySectionLink: 'Link zum Abschnitt kopieren',
  viewSource: 'Quelltext anzeigen',
  linkCopied: 'Link in die Zwischenablage kopiert',

  // file browser
  openArticle: 'Artikel öffnen',
  selectArticle: 'Wählen Sie einen Artikel für die Vorschau aus',

  // overlay dialog
  close: 'Schließen',
  hover: 'Schwebt ein/aus',

  // tour display
  guidedTour: 'Guided Tour',
  hide: 'Ausblenden',
  needs: 'Bedürfnisse',

  //Competency Indicator
  details: 'Einzelheiten',
  concepts: 'Konzepte',
  review: 'Rezension',

  // self assessement
  smileyTooltips: {
    Remember: {
      '2': 'Ich kann mich komplett daran erinnern',
      '1': 'Ich kann mich größtenteils daran erinnern',
      '0': 'Ich bin mir nicht sicher, ob ich mich daran erinnern kann',
      '-1': 'Ich kann mich kaum daran erinnern',
      '-2': 'Ich kann mich überhaupt nicht daran erinnern',
    },
    Understand: {
      '2': 'Ich verstehe es komplett',
      '1': 'Ich verstehe es größtenteils',
      '0': 'Ich bin mir nicht sicher, ob ich es verstehe',
      '-1': 'Ich verstehe es kaum',
      '-2': 'Ich verstehe es überhaupt nicht',
    },
  },
  enableDisableFilter: 'Klicken Sie zum Aktivieren/Deaktivieren von Filtern.',
  iconHovers: {
    Remember: 'Ich erinnere mich ',
    Understand: 'Ich verstehe es',
    Apply: 'Ich kann es anwenden',
    Analyse: 'Ich kann analysieren',
    Evaluate: 'Ich kann bewerten',
    Create: 'Ich kann Neues erstellen',
  },
  quiz: {
    youScored: 'Sie haben $1 von $2 Punkten erreicht.',
    feedbackAwaited: 'Quiz eingereicht. Rückmeldung erwartet.',
    of: 'von',
    problem: 'Aufgabe',
    finish: 'Beenden',
    prev: 'Vorherige',
    next: 'Nächste',
    didntAnswer: 'Sie haben $1 Aufgaben nicht beantwortet. ',
    sureFinish: 'Sind Sie sicher, dass Sie den Vorgang abschließen möchten?',
    showSolution: 'Lösung anzeigen',
    hideSolution: 'Lösung ausblenden',
    practiceProblem: 'Übungsprobleme anzeigen',
    syllabus: 'Lehrplan',
    adventurous: 'Ich bin abenteuerlustig',
    forMe: 'Für mich',
    syllabusTooltip: 'Übungsprobleme, die mit diesem Lehrplan abgestimmt sind.',
    adventurousTooltip:
      'Fordern Sie sich mit Problemen heraus, die über den Lehrplan hinausgehen - erweitern Sie Ihren Horizont.',
    forMeTooltip:
      'Personalisierte Probleme, die auf Ihren Lernfortschritt und Ihre Kompetenzen zugeschnitten sind.',
    hideProblems: 'Probleme ausblenden',
    hideForMe: 'Für mich ausblenden',
    hidepracticeProblem: 'Übungsprobleme ausblenden',
    NoPracticeProblemsForMe: 'Keine personalisierten Übungsprobleme für diesen Abschnitt verfügbar',
    NoPracticeProblemsAll: 'Keine Übungsprobleme für diesen Abschnitt verfügbar',
    answer: 'Antwort',
    point: 'Punkt',
    submit: 'Senden',
    reset: 'Zurücksetzen',
    feedback: 'Feedback',
    showOlderAnswers: 'Ältere Antworten anzeigen',
    history: 'Geschichte',
    saveAndSubmitAPeerReview: 'Speichern & Einreichen einer Peer-Review-Anfrage',
    saveAndSubmitAInstructorReview:
      'Speichern & Einreichen einer Beurteilungsanfrage für einen Kursleiter',
    save: 'retten',
    discard: 'abwerfen',
    saveAndGrade: 'Speichern und bewerten',
    yourAnswer: 'Ihre Antwort',
    yourAnswerWithIdx: 'Ihre Antwort ($1 von $2)',
  },
  practiceProblems: {
    clickSection:
      'Klicken Sie auf einen Abschnitt im Inhaltsverzeichnis, um die damit verbundenen Probleme anzuzeigen.',
    problemsFor: 'Aufgaben für',
    warning:
      'Hinweis: Diese Probleme dienen nur dazu, Ihnen beim Lernen zu helfen. Sie decken das Kursmaterial nicht vollständig ab und es gibt keine Garantie dafür, dass die Probleme korrekt sind oder dass sie repräsentativ für die Art von Problemen sind, die in den Tests oder der Prüfung behandelt werden.',
    closeAllPracticeProblems: 'Alle Übungsaufgaben schließen',
    showAllPracticeProblems: 'Alle Übungsaufgaben anzeigen',
  },
  problemFilter: {
    filterProblems: 'Probleme filtern',
    showingAll: 'Alle Probleme anzeigen',
    showingType: 'Zeige $1 Probleme',
    filterTitle: 'Probleme filtern',
    quiz: 'Quiz',
    homework: 'Hausaufgaben',
    exam: 'Prüfung',
    uncategorized: 'Unkategorisiert',
    cancel: 'Abbrechen',
    apply: 'Anwenden',
  },
};
